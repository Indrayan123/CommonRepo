...
node(env.ENV_Name)
{
    ...
    stage ('Compile And Deploy To The Environment')
    {
        ...
        dir(path:"${pomLoc}")
        {
            env.scriptOp =sh ('#!/bin/sh -e\n'+ "${env.Maven_Path}/mvn  -Dmaven.ext.class.path="/opt/schneider/applications/SOAFoundation/lib/FndIntgSoaExtensions.jar" pre-integration-test -Dsoapassword=${env.soapass} -DserverURL=${env.soaurl} -Duser=${env.soauser}")
        }
