...
node(env.ENV_Name)
{
    ...
    stage ('Compile And Deploy To The Environment')
    {
        ...
        dir(path:"${pomLoc}")
        {
            env.scriptOp =sh ('#!/bin/sh -e\n'+ "/opt/oracle/middleware/oracle_common/modules/org.apache.maven_3.2.5/bin/mvn -X pre-integration-test -Dsoapassword=${env.soapass} -DserverURL=${env.soaurl} -Duser=${env.soauser}")
        }
